...
similarity_df = pd.read_csv("map/Korean_Cities_SimilarityMatrix_Penalty0.csv", index_col=0)

# 도시 위도/경도 좌표 불러오기 (index: 도시 이름)
city_coords_df = pd.read_csv("map/Korean_Cities_100_with_Coordinates.csv")  # columns: City, Latitude, Longitude
...
for city in ["서울", "부산", "대전"]:
    if city in coord_map:
        ...
        if city=="서울":
            ...
            city = "Daejeon"
        ax.text(
            lon + 0.1, lat + 0.1,  # → 위치 조정: 오른쪽 위로 충분히 떨어뜨림
            city,
            fontsize=12,          # → 크기 키움
            fontweight='bold',    # → 진하게
...
plt.title("Similarity Graph of Korean Cities (Highlight: Seoul, Busan, Daejeon)")
plt.show()
